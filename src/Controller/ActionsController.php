...
class ActionsController extends AppController
{
    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->Actions->save($action)) {
                $this->Flash->success(__('The {0} has been saved.', 'Action'));

                return $this->redirect(['action' => 'index']);
            }
            $this->Flash->error(__('The {0} could not be saved. Please, try again.', 'Action'));
        }
    ...
    public function delete($id = null)
    {
        ...
        if ($this->Actions->delete($action)) {
            $this->Flash->success(__('The {0} has been deleted.', 'Action'));
        } else {
            $this->Flash->error(__('The {0} could not be deleted. Please, try again.', 'Action'));
        }
